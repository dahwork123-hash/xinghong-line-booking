  ...
  if (tab === "schedules") {
    ...
    const chips = (day) =>
      (r.isoWeekdays[day] || [])
        ...
        .join("");
    body = `<div class="studio"><div class="studio-hero"><h1>處長與面試時間</h1><p>先把每週固定面試時間排上去，再改處長姓名。求職者在 LINE 只會看到「套用」之後的時間。</p></div><div class="studio-steps"><span><i class="step-n">1</i>每週幾點到幾點：點時間可改處長，點＋可加時段</span><span><i class="step-n">2</i>誰來面試：點卡片就能改姓名</span></div><div class="week-toolbar"><div class="office-pills">${pills}</div><label>這個組每場最多幾人<input id="board-capacity" type="number" min="1" max="99" value="${r.capacity}"></label></div><p class="hint-card">${officeLabel(r.officeId)}${r.pool === "admin" ? "行政（找主管）" : "一般職缺"}。點色塊可改「這場由誰面試」或刪掉；週末沒排就會是空的。</p><div class="week-board">${[1, 2, 3, 4, 5, 6, 7].map((d) => `<section class="day-col"><h3>星期${dayNames[d]}<small>${(r.isoWeekdays[d] || []).length}場</small></h3>${chips(d)}<button class="ghost-add" data-action="add-slot" data-value="${d}">＋ 加時段</button></section>`).join("")}</div><div class="people-grid">${people}<button class="person-add" data-action="add-director">＋ 新增處長或主管</button></div><div class="studio-foot"><label>何時開始用新時間<input id="board-effective" type="date" min="${me.today}" value="${board.effectiveDate}"></label>${button("儲存草稿", "schedule-draft")}${button("預覽並套用給求職者", "schedule-preview", "", "primary")}<small>已有預約不會被搬走。單次停辦請用場次總覽。最後操作：${esc(settings.schedules.actor)}</small></div></div>`;
  }
  if (tab === "templates") {
